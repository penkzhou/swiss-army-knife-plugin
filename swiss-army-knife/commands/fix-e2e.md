...
# Bugfix E2E Workflow v2.1

基于测试失败的端到端用例，执行标准化 bugfix 流程。

**宣布**："我正在使用 Bugfix E2E v2.1 工作流进行问题修复。"
...
4. **提取技术栈配置**：从合并后的配置中提取 `stacks.e2e` 部分
...
| 变量名 | 配置路径 | 说明 |
| ------ | -------- | ---- |
| `config.test_command` | `stacks.e2e.test_command` | 测试命令 |
| `config.lint_command` | `stacks.e2e.lint_command` | Lint 命令 |
| `config.docs.bugfix_dir` | `stacks.e2e.docs.bugfix_dir` | Bugfix 文档目录 |
| `config.docs.best_practices_dir` | `stacks.e2e.docs.best_practices_dir` | 最佳实践目录 |
| `config.docs.search_keywords` | `stacks.e2e.docs.search_keywords` | 文档搜索关键词 |
| `config.error_patterns` | `stacks.e2e.error_patterns` | 错误模式定义 |
...
## 项目路径
- bugfix 文档: {e2e["docs"]["bugfix_dir"]}
- troubleshooting: {e2e["docs"]["best_practices_dir"]}/troubleshooting.md
"""
```
...
使用 Task tool 调用 e2e-error-analyzer agent，prompt 示例：
